...
> 런타임 기준 원본은 `workers/nightly/src/nightly-prompt.js`의 `nightly-v1`이다. 이 문서는 사람이 읽는 설계 설명이며, 변경 시 런타임 코드와 회귀 테스트를 함께 갱신한다.

하루치 대화 로그를 기본 **한 번의 LLM 호출**로 (1) 구조화 데이터로 추출하고 (2) 일기로 작성합니다. 검증 실패 시 정제된 오류 경로만으로 교정 호출을 최대 한 번 추가합니다.
